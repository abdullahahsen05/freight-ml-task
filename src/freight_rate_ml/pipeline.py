...
from .reporting import generate_report, package_versions
...
def run(project_root: str | Path = ".") -> None:
    ...
    p = paths(root)
    _ensure_dirs(root)
    _write_status(root, "Started pipeline; bootstrap in progress.")
    ...
    _write_status(root, "Input preservation complete. Data audit in progress.")
    ...
    _write_status(root, "Data audit complete. Model comparison in progress.")
    ...
    _write_status(root, f"Full model comparison complete. Selected full model: {selected_full}.")
    ...
    _write_status(root, "Validation predictions validated.")
    ...
    _write_status(root, "Supplied scorer passed and chart generated.")

    _write_prediction_summary(root, train, final_preds, dec_preds, manifest)
    report = generate_report(root)
    _verify_report(report)
    _write_status(root, "Report generated.")

    _write_final_readme(root, selection)
    _write_status(root, "Pipeline complete.")
...
def _write_status(root: Path, text: str) -> None:
    path = root / "artifacts" / "manifests" / "run_status.log"
